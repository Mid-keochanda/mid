    ...
    <div className="min-h-screen bg-slate-50 p-1 md:p-1 text-slate-700 font-sans antialiased">
      ...
      {isRejectModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-sm rounded-xl shadow-2xl p-5 border border-slate-200 animate-in zoom-in-95 duration-200">
            ...
            <textarea
              className="w-full h-24 p-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-500/10 focus:border-red-400 outline-none transition-all resize-none text-sm font-medium"
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-3 bg-white p-3 rounded-xl border border-orange-600 shadow-sm">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 bg-orange-900 rounded-lg flex items-center justify-center text-white shadow-sm">
              <FiCommand size={20} />
            </div>
            <div>
          ...
          <button 
            onClick={fetchData} 
            className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 text-slate-600 border border-slate-200 rounded-lg text-[11px] font-bold hover:border-blue-500 hover:text-blue-600 transition-all active:scale-95 group"
          >
          ...
          {data.length > 0 ? data.map((item) => {
            ...
            return (
              <div key={item.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col lg:flex-row hover:border-blue-200 transition-all group">
                ...
                <div className="p-4 flex flex-row lg:flex-col justify-center gap-2 bg-white lg:w-36 border-t lg:border-t-0 lg:border-l">
                  <button 
