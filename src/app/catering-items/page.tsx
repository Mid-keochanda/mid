...
export default function CateringItemsPage() {
  ...
  // ປັບ State ເບື້ອງຕົ້ນໃຫ້ກົງກັບຟີວໃໝ່
    ...
    <div className="space-y-8 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-6">
          <div className="w-16 h-16 bg-[#108553] rounded-2xl flex items-center justify-center text-white shadow-lg">
            <FiBox size={30} />
          </div>
          <div>
            <h1 className="text-2xl font-black text-[#1A2233]">ຈັດການ <span className="text-[#108553]">ສິ່ງຂອງ</span></h1>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input 
              placeholder="ຄົ້ນຫາ..." 
              className="bg-white border border-slate-200 pl-11 pr-5 py-3 rounded-xl w-72 shadow-sm focus:ring-2 focus:ring-emerald-500/10 outline-none text-sm font-medium"
              value={searchQuery}
          ...
          <button 
            ...
            className="bg-[#101828] text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-slate-800 transition-all active:scale-95"
          >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="px-8 py-5 text-[13px] font-black uppercase text-slate-400">ID</th>
                <th className="px-8 py-5 text-[13px] font-black uppercase text-slate-400">ຊື່ລາຍການ</th>
                <th className="px-8 py-5 text-[13px] font-black uppercase text-slate-400 text-center">ຫົວໜ່ວຍ</th>
                <th className="px-8 py-5 text-[13px] font-black uppercase text-slate-400 text-center">ສະຖານະ</th>
                <th className="px-8 py-5 text-[13px] font-black uppercase text-slate-400">ວັນທີບັນທຶກ</th>
                <th className="px-8 py-5 text-[13px] font-black uppercase text-slate-400 text-right">ຈັດການ</th>
              </tr>
            </thead>
              ...
              {filteredItems.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/30 transition-colors group">
                  <td className="px-8 py-5">
                    <span className="font-mono font-bold text-slate-400 bg-slate-100 px-2 py-1 rounded-md">#{item.id}</span>
                  </td>
                  <td className="px-8 py-5 font-bold text-slate-700">{item.Name}</td>
                  <td className="px-8 py-5 text-center">
                    <span className="text-xs font-bold text-slate-500 bg-white border border-slate-100 px-3 py-1.5 rounded-lg shadow-sm">
                      {item.Unit}
                    </span>
                  </td>
                  <td className="px-8 py-5 text-center">
                    {item.isActive ? (
                      <span className="bg-[#E8F5EE] text-[#108553] px-3 py-1 rounded-full text-[11px] font-black border border-[#D1E9DB]">ເປີດໃຊ້</span>
                    ) : (
                      <span className="bg-slate-100 text-slate-400 px-3 py-1 rounded-full text-[11px] font-black">ປິດ</span>
                    )}
                  </td>
                  <td className="px-8 py-5 text-[12px] text-slate-400 font-medium">
                    <div className="flex flex-col gap-1">
                      <span className="flex items-center gap-1"><FiPlus className="text-emerald-500" /> {item.createdAt}</span>
                      <span className="flex items-center gap-1"><FiEdit3 className="text-blue-500" /> {item.updatedAt}</span>
                    </div>
                  </td>
          ...
          <div className="bg-white rounded-[2rem] w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSave} className="p-8 space-y-5">
              <div className="space-y-1.5">
                ...
                <input required className="w-full bg-slate-50 border border-slate-200 p-3.5 rounded-xl font-bold text-sm outline-none focus:border-emerald-500 transition-all text-slate-700" 
              ...
              <div className="flex gap-4 pt-6">
                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 bg-red-600 text-white py-4 rounded-2xl font-black uppercase text-[14px]">ຍົກເລີກ</button>
                <button type="submit" className="flex-1 bg-emerald-600 text-white py-4 rounded-2xl font-black uppercase text-[14px]">ບັນທຶກ</button>
              </div>
            </form>
          </div>
        </div>
